...
% Each loop creates a linear SVM for just 1 class vs all
% Then tests the SVM and saves the W and B to an array with the index being
% the category 
for i=1:num_categories
    
    % Sets all values of label to -1;
    labels = zeros(size(train_labels)) -1;
    
    % Sets currently compared category to +1 in labels 
    ...
    % Trains classifier for category i 
    [w(i,:), b(i)] = vl_svmtrain(train_image_feats', labels', LAMBDA);
end


% Works out the scores of each test sample compared to all of the
% classifieres (categories)
scores = w*test_image_feats' + b'; 
% Finds the highest score of each test sample and saves the category index
% for it
[~, categoryIndex] = max(scores);

% Uses the categoryIndex to associate it with the categories and returns it
predicted_categories = categories(categoryIndex);
